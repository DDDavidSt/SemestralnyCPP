...
public:
    PTNetwork(): busstops(*new std::map<int, BusStop>) , buslines(*new std::map<int,BusLine>){};
    PTNetwork(std::map<int,BusStop> stops, std::map<int, BusLine> lines, std::string ) : busstops(std::move(stops)), buslines(std::move(lines)) {};
